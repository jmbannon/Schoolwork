...
# Author:	Jesse Bannon				#
# Class:	TCSS 372 B				#
# Desc:		Project #2 - Matrix Calculator		#
#=======================================================#
...
matrix_initialize:
	...
	mul $t3, $s1, 4		# Jump amount to store MB column major
	add $t4, $t4, $zero	# Column counter for MB
	add $t5, $t5, $zero     # Amount to add to base of MB
...
multiply_loop:		
	...
	add $t4, $t4, $t3	# sum += A(m,n) * B(n,p)
        ...
        addi $s4, $s4, 4	# increment column(B)
        j multiply_loop
	...
	add $s3, $t0, $zero	# Restores current row (A)
        ...
        addi $s4, $s4, 4	# Increment to next value in MB (next column)
	...
	add $t0, $zero, $zero
	add $t1, $zero, $zero
	mul $t2, $s1, 4		# Jump amount to store MB column major
	add $t3, $zero, $zero

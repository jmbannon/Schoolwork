...
int main(int argc, char **argv) {
	...
	if (rank == 0) {
		...
		res = SparseMatrix_init(&c, a.nr_rows, b.nr_cols, a.nr_rows * b.nr_cols);
	...
	if (rank == 0) {
		...
		res = SparseMatrix_init(&c_merge, a.nr_rows, b.nr_cols, offset);
		CHECK_ZERO_ERROR_RETURN(res, "Failed to init matrix c_merge");

		c_merge.nr_elems = offset;
		for (int i = 0; i < nr_elems[0]; i++) {
			c_merge.row[i] = c.row[i];
			c_merge.col[i] = c.col[i];
			c_merge.data[i] = c.data[i];
		}
